...
const useEx = true;
function buildBase(cloudId, site) {
    if (useEx && cloudId)
    ...
    try {
        ...
        if (req.path.endsWith('/jira/transition')) {
            ...
            if (!issueKey || !transitionId) {
                res.status(400).json({ error: 'Missing issueKey/transitionId' });
                return;
            }
        ...
        if (req.path.endsWith('/jira/attach')) {
            ...
            if (!issueKey || !fileName || !fileBase64) {
                res.status(400).json({ error: 'Missing issueKey/fileName/fileBase64' });
                return;
            }
